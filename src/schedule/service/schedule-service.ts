...
export class ScheduleService implements IScheduleService {
  ...
  async listSchedules(query: GetSchedulesQueryDto): Promise<GetSchedulesResponseDto> {
   
    let fakeFilter = schedulesMock;
    if (query?.name) {
      fakeFilter = schedulesMock.filter((item) => item.name === query.name);
    } else if (query?.speciality) {
      fakeFilter = schedulesMock.filter((item) => item.speciality === query.speciality);
    } else if (query?.availableDate) {      
      fakeFilter = schedulesMock.filter((item) => item.available_schedules.includes(query.availableDate));
    }
