...
export class SchedulerController {
	constructor(@inject(ScheduleService.name) private readonly scheduleService: IScheduleService) {}

	async schedules(event: any) {
		try {						
			const result = await this.scheduleService.listSchedules(event.queryStringParameters);
